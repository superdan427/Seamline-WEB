...
import { escapeHtml, sanitizePhotoArray } from "../utils/sanitizer.js";
  ...
  const galleryHtml =
    safePhotos.length > 0
      ? `<div class="modal-gallery">
          ${safePhotos.map((src) => `<img src="${escapeHtml(src)}" alt="" />`).join("")}
         </div>`
      : "";

  // Security: Escape all user-provided content to prevent XSS attacks
  modalBody.innerHTML = `
    <div class="modal-title">${escapeHtml(place.name ?? "")}</div>
    <div class="modal-sub">${escapeHtml(place.category ?? "")}</div>
    ${place.pop_up ? `<p class="modal-description">${escapeHtml(place.pop_up)}</p>` : ""}
