...
import { sanitizeUrl, sanitizePhotoArray, createSafeImage } from "../utils/sanitizer.js";
...
async function loadPlace() {
  ...
  // Try to find by slug first (for text IDs like "whaleys_bradford")
  ...
  // If not found by slug, try by UUID (for new database IDs)
  if (!place && !error) {
    console.log("⚠️ Not found by slug, trying UUID...");
  ...
  if (!place) {
    ...
    // Optionally show a "Place not found" message to user
    return;
  }
...
function renderPlace(place) {
  ...
  // Website
  // Security: Sanitize website URL to prevent javascript: protocol injection
  ...
  if (place.website) {
    ...
    if (safeUrl) {
      ...
      websiteEl.textContent = formatWebsiteLabel(safeUrl);
  ...
  // Gallery (optional)
  // Security: Sanitize photo URLs to prevent javascript: protocol injection
  ...
  if (photos.length > 0) {
    ...
    // Sanitize photo URLs and create safe image elements
    const safePhotos = sanitizePhotoArray(photos);
    safePhotos.forEach((photoUrl) => {
      const img = createSafeImage(photoUrl, "Place photo");
...
function populatePlaceCollections() {
  ...
  placeCollectionSelect.innerHTML = "";

  // Add default option using DOM methods
  const defaultOption = document.createElement("option");
  defaultOption.value = "";
  defaultOption.textContent = "Choose a folder";
  placeCollectionSelect.appendChild(defaultOption);

  // Security: Use textContent to prevent XSS from collection names
